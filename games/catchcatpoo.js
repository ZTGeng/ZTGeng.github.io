...
var findPath = function(choices) {
	...
	while (choices.length > 0) {
		...
		if ($(grids[next]).hasClass('poo')) continue;
		if ($(grids[next]).hasClass('out')) return original;
...
var showNum = function(pos) {
	...
	if (num > 0) $(grids[pos]).html("<p>" + num + "</p>");
};

var reset = function() {
	$('.end').removeClass('end');
	$('.poo').removeClass('showPoo poo');
	$('.opened').removeClass('opened');
	grids.text("");
	for (var i = 0; i < 8; i++) {
		var random = Math.floor(Math.random() * 81);
		if (random != 40) $(grids[random]).addClass('poo');
	}
	for (var i = 0; i < 8; i++) {
		var random = Math.floor(Math.random() * 81);
